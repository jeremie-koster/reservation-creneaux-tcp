...
from src.repositories.transformations import create_df_from_parsing, clean_booking_slots, format_slots_data
...
def test_format_slots_data():
    input_path = "test/unit/data/repositories/input/slots_before_formatting.json"
    input_slots = pd.read_json(input_path)

    result = format_slots_data(input_slots)

    expected_path = "test/unit/data/repositories/output/slots_transformed.json"
    expected_df = pd.read_json(expected_path, keep_default_dates=True, convert_dates=["start_time"])
